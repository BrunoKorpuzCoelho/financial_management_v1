...
const ivaRateGroup =
  document.querySelector(".form-group:has(#iva-rate)") ||
  ivaRateSelect.closest(".form-group");
            ...
            .then((data) => {
              ...
              if (row) {
                ...
                if (remainingRows === 0 && currentPage > 1) {
                  ...
                  window.location.href = `/simple-sales/${getCompanyIdFromUrl()}?page=${
...
function loadFinancialData() {
  const company_id = getCompanyIdFromUrl();
  console.log(
    "Carregando dados financeiros para empresa:",
    company_id,
    "mês:",
    currentMonth,
    "ano:",
  ...
  fetch(
    `/api/simple-financial-summary?company_id=${company_id}&month=${currentMonth}&year=${currentYear}`
  )
    .then((response) => response.json())
    .then((data) => {
